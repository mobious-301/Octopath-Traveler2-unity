...
void InitializeInputData(Varyings IN, half3 normalTS, out InputData inputData)
{
    ...
    // inputData.bakedGI=IN.uvMainAndLM.zww;
#endif
...
float4 getgi(Varyings input ){
...
#if defined(DYNAMICLIGHTMAP_ON)
    inputData.bakedGI = SAMPLE_GI(input.uvMainAndLM.zw, input.dynamicLightmapUV, input.vertexSH, inputData.normalWS);
    // inputData.bakedGI=.x;
#else
    inputData.bakedGI = SAMPLE_GI(input.uvMainAndLM.zw, input.vertexSH, inputData.normalWS);
    // inputData.bakedGI=float3(1,0,0);
#endif
    ...
    half4 giColor = UniversalFragmentPBR(inputData, 1,0,0,1,1,0,1);
    // giColor.xyz=inputData.bakedGI;
    return giColor;
}
...

{
    ...
    InitializeStandardLitSurfaceData(input.uvSplat01, surfaceData);
    // SurfaceData surfaceData;
...
// outColor= getgi(input);
outColor=1;
outColor.xyz=inputData.bakedGI;
...
//     splatControl.xyz = GetTriWeights(IN.normal);//####覆盖
...
//     // albedo = GetTriUV(IN.positionWS).xUV.xxy;
...
//     // brdfData.albedo.rgb = float4(alpha,0,0,0);
...
//     // outColor = half4(inputData.bakedGI, 1.0h);
//     // outColor = half4(1,0,0, 1.0h);
//     // outColor.xyz =inputData.bakedGI;
//     // outColor.xyz =inputData.normalWS;
