...
occlusion=1;
    ...
    SplatmapFinalColor(color, inputData.fogCoord);
    // color.xyz = inputData.bakedGI = SAMPLE_GI(IN.uvMainAndLM.zw, IN.dynamicLightmapUV, 0, inputData.normalWS);
    ...
    Varyings input =IN;
    #if defined(DYNAMICLIGHTMAP_ON)
    color = SAMPLE_GI(input.uvMainAndLM.zw, input.dynamicLightmapUV, input.vertexSH, inputData.normalWS);
    // inputData.bakedGI=.x;
#else
    color.xyz = SAMPLE_GI(input.uvMainAndLM.zw, input.vertexSH, inputData.normalWS);
    // inputData.bakedGI=float3(1,0,0);
#endif
    // color.xy=IN.uvMainAndLM.zw;
    // color =unity_SHBr;
